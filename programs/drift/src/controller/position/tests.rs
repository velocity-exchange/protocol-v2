...
fn amm_perp_ref_offset() {
    ...
    assert_eq!(b, 7103317); // wider long spread from funding bias when AMM is net long
    assert_eq!(a, 7110447); // ask adjusted for updated FUNDING_RATE_OFFSET_DENOMINATOR (3333)
    assert_eq!(
        perp_market.amm.historical_oracle_data.last_oracle_price,
        7101600
    );
    assert_eq!(perp_market.amm.reference_price_offset, 742); // adjusted for ~10.95% APR offset denominator (3333)
